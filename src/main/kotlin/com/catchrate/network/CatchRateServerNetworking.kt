...
object CatchRateServerNetworking {
    ...
    fun initialize() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(WorldCatchRateRequestPayload.ID) { payload, context ->
            handleWorldCatchRateRequest(context.player(), payload)
        }
    }
    ...
    /**
     * Handle a world (out-of-combat) catch rate request.
     * Looks up the PokemonEntity by entity ID in the server world and calculates
     * the catch rate with full server-side species data.
     * This fixes the bug where non-host multiplayer clients see the same catch rate
     * for every Pokemon because client-side species data isn't fully synced.
     */
    private fun handleWorldCatchRateRequest(player: ServerPlayerEntity, request: WorldCatchRateRequestPayload) {
        try {
            val entity = player.serverWorld.getEntityById(request.entityId)
            ...
            val world = player.world
            val ctx = BallContextFactory.fromPokemon(pokemon, player, world, inBattle = false, turnCount = 0, activeBattler = null)
            ...
            CatchRateDisplayMod.debug("=== WORLD CATCH RATE (out-of-combat) ===")
            CatchRateDisplayMod.debug("Pokemon: ${pokemon.species.name} Lv${pokemon.level} | HP: $currentHp/$maxHp | Base: $baseCatchRate")
            CatchRateDisplayMod.debug("Ball: ${ballId.path} ${ballMultiplier}x (valid=$ballIsValid) | Status: ${status?.name?.path ?: "none"} ${bonusStatus}x | LvBonus: $bonusLevel")
            CatchRateDisplayMod.debug("Out-of-combat: 0.5x | ModifiedRate: $modifiedCatchRate | Shake: $shakeProbability/65537 | Catch: $catchChance%")
            ...
            sendResponse(player, response)
            CatchRateDisplayMod.debug("World sent: ${pokemon.species.name} Lv${pokemon.level}, $ballName ${ballMultiplier}x = ${String.format("%.1f", catchChance)}%")
            
        } catch (e: Exception) {
            CatchRateDisplayMod.LOGGER.error("Error calculating world catch rate", e)
        }
    }
